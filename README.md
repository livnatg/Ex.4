#עבודה 4 – יישום שיטות לניתוח נתונים
## מגישות רז שיין 201054103 , ליבנת גרשוני 301792792 ##
###Text Mining ###
להלן הקוד שבעזרתו חזינו את רלונטיות המסמך לשאילתא:
הקוד נכתב בשפת R
```{r} 
...
``` 
תחילה טענו את הקבצים והחבילות הרלונטיות, כמו כן הורדנו את השדות מסט האימון שלא נמצאים בסט הבדיקה
הטיפול בסט הנתונים:
```{r} 
...
``` 
השתמשנו במספר שיטוות לניקוי הדאטה כמו: הורדת מספרים, מילים נוצות, רווחים וכו'
לאחר מכן,יצרנו עבור כל תכונה קורפרוס לפי 
document term:
...
``` 
והכנו את סט הנותנים לקראת הרצת המודל, בחרנו במודל SVM
הסיבה לכך הייתה שהתוצאות שקיבלנו ממודל זה היו הטובות ביותר (השוונו מול רנדום פורסט)

לאחר מכן הרצנו את המודל ואת הפרדיקציה
```{r} 
...
``` 
כתבנו את התוצאות לקובץ והעלנו אותו אל האתר/
ניתן לראות את קובץ התוצאות 
[כאן](https://github.com/razshain/Ex4/blob/master/ass4_model.csv)

קיבלנו את המיקום 871 לפי הדירוג:
![alt text](https://github.com/razshain/Ex4/blob/master/score.PNG "p1")
